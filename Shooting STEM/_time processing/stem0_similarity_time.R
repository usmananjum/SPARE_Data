...
stem0_28_temp <- read.csv(file="Shooting STEM/stem0/stem28_2.csv",header=TRUE);

stem0_28ne <- subset(stem0_28_temp, as.POSIXct(stem0_28_temp$created) < as.POSIXct(event_time_stem));
stem0_28e <- subset(stem0_28_temp, as.POSIXct(stem0_28_temp$created) > as.POSIXct(event_time_stem));
...
myCorpus0_28e <- Corpus(VectorSource(stem0_28e$text)); 
...
freq.terms0_28e <- findFreqTerms(tdm0_28e, lowfreq = 200);
term.freq0_28e <- rowSums(as.matrix(tdm0_28e));
term.freq0_28e <- subset(term.freq0_28e, term.freq0_28e >= 200);
df0_28e <- data.frame(ind=rep("0_28e",length(term.freq0_28e)), term = names(term.freq0_28e), freq = term.freq0_28e)
ggplot(df0_28e, aes(x=term, y=freq)) + geom_bar(stat="identity")+xlab("Terms") + ylab("Count") + coord_flip()+theme(axis.text=element_text(size=20))
...
# findAssocs(tdm0_28e, "school", 0.2) # which words are associated with 'shooting'?
...
tweet0_28e <- textclean2(stem0_28e$text);
...
# senti0_28e <- inner_join(token0_28e, get_sentiments("loughran")) %>% count(sentiment);
senti0_28e_temp <- inner_join(token0_28e, get_sentiments("bing"));
senti0_28e <- data.frame(table(senti0_28e_temp$sentiment));
colnames(senti0_28e) <- c("sentiment","n");
senti0_28e$percent = (senti0_28e$n/sum(senti0_28e$n))*100;
#Plotting the sentiment summary 
ggplot(senti0_28e, aes(sentiment, percent)) + geom_bar(aes(fill = sentiment), position = 'dodge', stat = 'identity') + 
...
sentiments0_28e <- sentiment(stem0_28e$text)
...
sentiments0_28e$time <- as.POSIXct(round(as.POSIXct(stem0_28e$created,format = "%Y-%m-%d %H:%M:%S"),"hours"));
...
myCorpus0_28ne <- Corpus(VectorSource(stem0_28ne$text)); 
...
freq.terms0_28ne <- findFreqTerms(tdm0_28ne, lowfreq = 200);
term.freq0_28ne <- rowSums(as.matrix(tdm0_28ne));
term.freq0_28ne <- subset(term.freq0_28ne, term.freq0_28ne >= 200);
df0_28ne <- data.frame(ind=rep("0_28ne",length(term.freq0_28ne)), term = names(term.freq0_28ne), freq = term.freq0_28ne)
ggplot(df0_28ne, aes(x=term, y=freq)) + geom_bar(stat="identity")+xlab("Terms") + ylab("Count") + coord_flip()+theme(axis.text=element_text(size=20))
...
# findAssocs(tdm0_28ne, "school", 0.2) # which words are associated with 'shooting'?
...
tweet0_28ne <- textclean2(stem0_28ne$text);
...
#Matching sentiment words from the sentiment lexicon
# senti0_28ne <- inner_join(token0_28ne, get_sentiments("loughran")) %>% count(sentiment);
senti0_28ne_temp <- inner_join(token0_28ne, get_sentiments("bing"));
senti0_28ne <- data.frame(table(senti0_28ne_temp$sentiment));
colnames(senti0_28ne) <- c("sentiment","n");
senti0_28ne$percent = (senti0_28ne$n/sum(senti0_28ne$n))*100;
#Plotting the sentiment summary 
ggplot(senti0_28ne, aes(sentiment, percent)) + geom_bar(aes(fill = sentiment), position = 'dodge', stat = 'identity') + 
  ggtitle("Sentiment analysis") + coord_flip() +
  theme(legend.position = 'none', plot.title = element_text(size=18, face = 'bold'),axis.text=element_text(size=18),axis.title=element_text(size=14,face="bold"))

# Sentiment Analysis (alternative)
sentiments0_28ne <- sentiment(stem0_28ne$text)
...
sentiments0_28ne$time <- as.POSIXct(round(as.POSIXct(stem0_28ne$created,format = "%Y-%m-%d %H:%M:%S"),"hours"));
result0_28ne <- aggregate(score ~ time, data = sentiments0_28ne, sum)
ggplot(result0_28ne,aes(time,score)) + geom_line() + geom_vline(xintercept = event_time_stem) + 
